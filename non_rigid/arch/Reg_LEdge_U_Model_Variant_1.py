...
class ResUnet(nn.Module):
    def __init__(self, shape, channel, filters=[8, 16, 32, 32, 64]):
        ...
        self.edge_conv4 = EdgeDetectionModule3D(filters[4], filters[4])
        self.residual_conv4_cat = ResidualConv(128, 64,1,1)
        
        self.residual_conv5 = ResidualConv(filters[1], filters[0], 2, 1)

        self.aspp_bridge = ASPP(filters[4], filters[4])

        self.upsample1 = Upsample_(2)
        self.up_residual_conv1 = ResidualConv(filters[4] + filters[3], filters[3], 1, 1)

        self.upsample2 = Upsample_(2)
        self.up_residual_conv2 = ResidualConv(filters[3] + filters[3], filters[1], 1, 1)
    ...
    def forward(self, x):
        ...
        x2_e = self.edge_conv1(x2)       
        x2_cat = torch.cat([x2, x2_e], dim=1)
        x2_cat = self.residual_conv1_cat(x2_cat) # 16,80,96,112


        x3 = self.residual_conv2(x2_cat)
        x3_e = self.edge_conv2(x3)
        x3_cat = torch.cat([x3, x3_e], dim=1)
        x3_cat = self.residual_conv2_cat(x3_cat) # 32,40,48,56


        x4 = self.residual_conv3(x3_cat)
        x4_e = self.edge_conv3(x4)
        x4_cat = torch.cat([x4, x4_e], dim=1)
        x4_cat = self.residual_conv3_cat(x4_cat) #32,20,24,28
        
        
        x5 = self.residual_conv4(x4_cat)
        x5_e = self.edge_conv4(x5)
        x5_cat = torch.cat([x5, x5_e], dim=1)
        x5_cat = self.residual_conv4_cat(x5_cat) #1,64,10,12,14
        x5_bridge = self.aspp_bridge(x5_cat)
        
        
        x6 = self.upsample1(x5_bridge)
        x6 = torch.cat([x6, x4_cat], dim=1)
        x6 = self.up_residual_conv1(x6) #1,32,20,24,28
        
        x7 = self.upsample1(x6)
        x7 = torch.cat([x7, x3_cat], dim=1)
        x7 = self.up_residual_conv2(x7) #1,32,40,48,56
        
        x8 = self.upsample1(x7)
        x8 = torch.cat([x8, x2_cat], dim=1)
        x8 = self.up_residual_conv3(x8) #1,16,80,96,112
        
        x9 = self.upsample1(x8)
        x9 = torch.cat([x9, x1], dim=1)
        x9 = self.up_residual_conv4(x9) #1,16,80,96,112
        
        x10 = self.aspp_out(x9)
        flow = self.output_layer(x10)
